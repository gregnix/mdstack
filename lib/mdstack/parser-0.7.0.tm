...
package provide mdstack::parser 0.7.0
                ...
                if {[regexp {^([[:space:]]*)(\*|-|\+|[0-9]+[.)])[[:space:]]+} $next -> nextWs]} {
                    ...
                    incr i
                          ...
                          && (($baseIndent > 0
                              ...
                              || ($baseIndent == 0
                                  && (![regexp {^(    |\t)} $next]
                                      || [mdstack::parser::_isSubBlockLine $next $contentIndent])))} {
...
# Remove up to n columns of leading indent (tab = 4 columns).
proc mdstack::parser::_deindent {line n} {
    set col 0; set idx 0; set L [string length $line]
    while {$col < $n && $idx < $L} {
        set c [string index $line $idx]
        if {$c eq " "} { incr col; incr idx } \
        elseif {$c eq "\t"} { incr col 4; incr idx } else break
    }
    return [string range $line $idx end]
}

# Is this item-continuation line a nested BLOCK (indented code / block quote /
# fence once de-indented by the item's content column) rather than plain lazy
# paragraph text?
proc mdstack::parser::_isSubBlockLine {line contentIndent} {
    regexp {^([[:space:]]*)} $line -> _w
    if {[string length $_w] < $contentIndent} { return 0 }
    set body [mdstack::parser::_deindent $line $contentIndent]
    if {[regexp {^(    |\t)\S} $body]} { return 1 }
    if {[string index $body 0] eq ">"} { return 1 }
    if {[regexp {^(```|~~~)} $body]} { return 1 }
    return 0
}

proc mdstack::parser::_mkListItem {paras subLines checked {contentIndent 0}} {
    set itemBlocks {}
    foreach para $paras {
    ...
    if {[llength $subLines] > 0} {
        set _fnb ""
        foreach _l $subLines { if {[string trim $_l] ne ""} { set _fnb $_l; break } }
        if {[regexp {^[[:space:]]*(\*|-|\+|[0-9]+[.)])[[:space:]]+} $_fnb]} {
            lappend itemBlocks [mdstack::parser::parseListLines $subLines]
        } else {
            set _body {}
            foreach _l $subLines { lappend _body [mdstack::parser::_deindent $_l $contentIndent] }
            set _subRef {}
            foreach _b [mdstack::parser::parseBlocks _body _subRef] {
                if {[dict exists $_b type]} { lappend itemBlocks $_b }
            }
        }
    }
    ...
    foreach line $lines {
        ...
        if {$lineIndent <= $baseIndent && $hasMarker &&
            ...
            if {$hasItem} {
                ...
                lappend items [mdstack::parser::_mkListItem $curParas $subLines $currentChecked $curContentIndent]
            }
            set currentChecked ""
            ...
            set curParas {}
            if {[regexp {^([[:space:]]*)(\*|-|\+|[0-9]+[.)])([[:space:]]+)} $line -> _cw _cm _cs]} {
                set curContentIndent [expr {[string length $_cw] + [string length $_cm] + [string length $_cs]}]
            } else { set curContentIndent [expr {$baseIndent + 2}] }
            ...
            if {$seenSubItem} { lappend subLines "" }
                  ...
                  && ![mdstack::parser::_isSubBlockLine $line $curContentIndent]} {
            if {$blankPending} {
