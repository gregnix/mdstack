...
> **API reference:** [English version](../en/mdviewer.md)
...
## Intern: Index-Pattern für Link-Tags

Link-Tags verwenden `"end -1 chars"` (nicht `end`) für die Index-Erfassung:
...
**Warum:** Das Text-Widget hat immer ein implizites Newline am Ende.
`end` zeigt *nach* diesem Newline. Nach dem Einfügen von Inline-Inhalt
kann `start >= end` gelten — `tag add` fügt dann stillschweigend keinen
Bereich hinzu und das Klick-Binding ist unerreichbar.
`"end -1 chars"` zeigt auf das letzte echte Zeichen und bleibt
nach Einfügungen stabil.

**Symptom wenn fehlerhaft:** Links sind blau und unterstrichen, aber
Klicks haben keine Wirkung. `$t tag ranges link1` gibt leer zurück.
