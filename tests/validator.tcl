...
# No type field
set errs [mdstack::validator::validate [dict create blocks {}]]
assert "B1: missing type detected" {[llength $errs] > 0}

# Wrong root type
set errs [mdstack::validator::validate [dict create type paragraph content {}]]
assert "B2: wrong root type detected" {[llength $errs] > 0}

# Missing blocks
set errs [mdstack::validator::validate [dict create type document version 1 meta {} reflinks {}]]
...
set ast [mdstack::parser::parse $md]
set r [mdstack::validator::report $ast]
assert "C1: report valid" {[string match "*valide*" $r]}

set r [mdstack::validator::report [dict create type paragraph]]
...
set errs_normal [mdstack::validator::validate $ast]
set errs_strict [mdstack::validator::validate $ast -strict]
...
set errs [mdstack::validator::validate $ast -strict]
